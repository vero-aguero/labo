...
PARAM  <- list()
PARAM$experimento <- "HT9411_sinCanaritos"

PARAM$exp_input  <- "TS9311_sinCanaritos"
...
exp_log  <- function( reg, arch=NA, folder="./exp/ExperimentoColectivo/WithUnderSampling/", ext=".txt", verbose=TRUE )
{
  archivo  <- arch
...
dataset_input  <- paste0( "./exp/ExperimentoColectivo/WithUnderSampling/", PARAM$exp_input, "/dataset_training.csv.gz" )
...
dir.create( paste0( "./exp/ExperimentoColectivo/WithUnderSampling/", PARAM$experimento, "/"), showWarnings = FALSE )
setwd(paste0( "./exp/ExperimentoColectivo/WithUnderSampling/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO
