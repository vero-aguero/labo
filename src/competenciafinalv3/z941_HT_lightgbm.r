...
PARAM  <- list()
PARAM$experimento <- "HT9410_v2"

PARAM$exp_input  <- "TS9310_v2"
...
exp_log  <- function( reg, arch=NA, folder="./exp/CompetenciaFinal/", ext=".txt", verbose=TRUE )
{
  archivo  <- arch
...
dataset_input  <- paste0( "./exp/CompetenciaFinal/", PARAM$exp_input, "/dataset_training.csv.gz" )
...
dir.create( paste0( "./exp/CompetenciaFinal/", PARAM$experimento, "/"), showWarnings = FALSE )
setwd(paste0( "./exp/CompetenciaFinal/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO
